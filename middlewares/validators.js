...
/**
 * 🛡️ DITANGANI DI SINI - Bagian 2 (implementasi mandiri): Buku Tamu.
 *
 * Validasi (server-side, TIDAK bisa dibypass lewat disable JS/Postman) +
 * Sanitasi (trim + rapihin spasi ganda) buat form komentar di /guestbook.
 *
 * Sengaja PISAH tanggung jawab dari escape:
 * - Di sini: cuma trim & normalize whitespace (SANITASI di titik INPUT)
 * - Escape karakter HTML (<, >, dll) BUKAN dikerjain di sini, tapi pas
 *   RENDER via <%= %> di views/guestbook.ejs (topik #3 - Escape HTML).
 *   Ini praktik yang disaranin OWASP: "encode on output", bukan disimpen
 *   dalam bentuk ter-escape dari awal - biar data mentah di DB tetep utuh
 *   dan gak numpuk banyak lapis encoding.
 */
const guestbookValidationRules = [
  body('name')
    .trim()
    .customSanitizer((value) => value.replace(/\s+/g, ' ')) // sanitasi: rapihin spasi ganda
